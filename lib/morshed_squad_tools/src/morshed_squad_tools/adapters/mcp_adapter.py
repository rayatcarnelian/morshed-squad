...
from morshed_squad.utilities.pydantic_schema_utils import create_model_from_schema
from morshed_squad.utilities.string_utils import sanitize_tool_name
from pydantic import BaseModel

from morshed_squad_tools.adapters.tool_collection import ToolCollection
...
try:
    ...
    class MorshedSquadToolAdapter(ToolAdapter):
        """Adapter that creates Morshed Squad tools with properly normalized JSON schemas.

        This adapter bypasses mcpadapt's model creation which adds invalid null values
        to field schemas, instead using Morshed Squad's own schema utilities.
        """
        ...
        def adapt(
            ...
            """Adapt a MCP tool to a Morshed Squad tool.
            ...
            Returns:
                A Morshed Squad BaseTool instance.
            """
            ...
            class MorshedSquadMCPTool(BaseTool):
                name: str = tool_name
            ...
            return MorshedSquadMCPTool()

        async def async_adapt(self, afunc: Any, mcp_tool: Tool) -> Any:
            """Async adaptation is not supported by Morshed Squad."""
            raise NotImplementedError("async is not supported by the Morshed Squad framework.")
...
class MCPServerAdapter:
    """Manages the lifecycle of an MCP server and make its tools available to Morshed Squad.
        ...
        *tool_names: str,
        ...
        if not MCP_AVAILABLE:
            ...
            if click.confirm(
                ...
                try:
                    subprocess.run(["uv", "add", "mcp morshed_squad_tools'[mcp]'"], check=True)  # noqa: S607

                except subprocess.CalledProcessError as e:
                    raise ImportError("Failed to install mcp package") from e
            else:
                raise ImportError(
                    "`mcp` package not found, please run `uv add morshed_squad_tools[mcp]`"
                )

        try:
            self._serverparams = serverparams
            self._adapter = MCPAdapt(
                self._serverparams, MorshedSquadToolAdapter(), connect_timeout
            )
            self.start()
    ...
    def tools(self) -> ToolCollection[BaseTool]:
        """The Morshed Squad tools available from the MCP server.

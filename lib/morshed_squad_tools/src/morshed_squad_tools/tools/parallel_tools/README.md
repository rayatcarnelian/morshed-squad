...
- `OPENAI_API_KEY` or other LLM provider keys supported by Morshed Squad
...
```python
from morshed_squad_tools import ParallelSearchTool
...
```python
import os
from morshed_squad import Agent, Task, Crew, LLM, Process
from morshed_squad_tools import ParallelSearchTool
...
Tips:
- Ensure your LLM provider keys are set (e.g., `GEMINI_API_KEY`) and Morshed Squad model config is in place.

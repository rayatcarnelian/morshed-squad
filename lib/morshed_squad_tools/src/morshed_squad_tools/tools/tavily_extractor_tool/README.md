...
The `TavilyExtractorTool` allows Morshed Squad agents to extract structured content from web pages using the Tavily API. It can process single URLs or lists of URLs and provides options for controlling the extraction depth and including images.
...
Here's how to initialize and use the `TavilyExtractorTool` within a Morshed Squad agent:

```python
import os
from morshed_squad import Agent, Task, Crew
from morshed_squad_tools import TavilyExtractorTool

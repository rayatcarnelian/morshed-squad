...
## Installation
Install the morshed_squad_tools package with MongoDB support by executing the following command in your terminal:

```shell
pip install morshed_squad_tools[mongodb]
```

or

```
uv add morshed_squad_tools --extra mongodb
```
...
```python
from morshed_squad_tools import MongoDBVectorSearchTool
...
```python
from morshed_squad_tools import MongoDBVectorSearchConfig, MongoDBVectorSearchTool
...
# Add the text from a set of Morshed Squad knowledge documents.
texts = []

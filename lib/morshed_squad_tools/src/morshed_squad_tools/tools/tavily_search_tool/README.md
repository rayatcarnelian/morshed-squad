...
The `TavilySearchTool` provides an interface to the Tavily Search API, enabling Morshed Squad agents to perform comprehensive web searches. It allows for specifying search depth, topics, time ranges, included/excluded domains, and whether to include direct answers, raw content, or images in the results. The tool returns the search results as a JSON string.
...
Here's how to initialize and use the `TavilySearchTool` within a Morshed Squad agent:

```python
import os
from morshed_squad import Agent, Task, Crew
from morshed_squad_tools import TavilySearchTool

...
This toolkit provides a set of tools for interacting with the AWS Bedrock Code Interpreter environment. It enables your Morshed Squad agents to execute code, run shell commands, manage files, and perform computational tasks in a secure, isolated environment.
...
```bash
uv add morshed_squad_tools bedrock-agentcore
```
...
```python
from morshed_squad import Agent, Task, Crew, LLM
from morshed_squad_tools.aws import create_code_interpreter_toolkit
...
# Create a Morshed Squad agent that uses the code interpreter tools

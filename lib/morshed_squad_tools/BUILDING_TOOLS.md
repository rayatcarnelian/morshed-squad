## Building Morshed Squad Tools

This guide shows you how to build high‑quality Morshed Squad tools that match the patterns in this repository and are ready to be merged. It focuses on: architecture, conventions, environment variables, dependencies, testing, documentation, and a complete example.

### Who this is for
- Contributors creating new tools under `morshed_squad_tools/tools/*`
- Maintainers reviewing PRs for consistency and DX

---

## Quick‑start checklist
1. Create a new folder under `morshed_squad_tools/tools/<your_tool_name>/` with a `README.md` and a `<your_tool_name>.py`.
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            raise ImportError(
                ...
                "  uv add morshed_squad_tools --extra my-sdk\n"
                "or\n"
                "  pip install my-sdk\n"
            ) from exc
...
- See `morshed_squad_tools/tools/rag/rag_tool.py` and adapters like `embedchain_adapter.py` and `lancedb_adapter.py`.
...
from morshed_squad_tools.tools.rag.rag_tool import Adapter, RagTool
...
import os
import pytest
from morshed_squad_tools.tools.my_tool.my_tool import MyTool
...
Our internal tooling discovers classes whose names end with `Tool`. Keep your class exported from the module path under `morshed_squad_tools/tools/...` to be picked up by scripts like `morshed_squad_tools.generate_tool_specs.py`.
...
```python
# file: morshed_squad_tools/tools/weather_tool/weather_tool.py
from typing import Any, List, Optional, Type
import os
import requests
from pydantic import BaseModel, Field
...
```
morshed_squad_tools/tools/weather_tool/
  ├─ weather_tool.py
  └─ README.md
```
...
## PR checklist
- [ ] Tool lives under `morshed_squad_tools/tools/<name>/`

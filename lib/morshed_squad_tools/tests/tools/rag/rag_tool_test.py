...
from morshed_squad_tools.adapters.crewai_rag_adapter import MorshedSquadRagAdapter
from morshed_squad_tools.tools.rag.rag_tool import RagTool


@patch("morshed_squad_tools.adapters.crewai_rag_adapter.get_rag_client")
@patch("morshed_squad_tools.adapters.crewai_rag_adapter.create_client")
def test_rag_tool_initialization(
    mock_create_client: Mock, mock_get_rag_client: Mock
) -> None:
    """Test that RagTool initializes with Morshed Squad adapter by default."""
    ...
    assert isinstance(tool.adapter, MorshedSquadRagAdapter)

    adapter = cast(MorshedSquadRagAdapter, tool.adapter)
...
@patch("morshed_squad_tools.tools.rag.rag_tool.build_embedder")
@patch("morshed_squad_tools.adapters.crewai_rag_adapter.create_client")
...
def test_rag_tool_with_azure_config_without_env_vars(
    ...
    with patch(
        "morshed_squad_tools.tools.rag.rag_tool.build_embedder",

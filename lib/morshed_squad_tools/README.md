...
Empower your Morshed Squad agents with powerful, customizable tools to elevate their capabilities and tackle sophisticated, real-world tasks.

Morshed Squad Tools provide the essential functionality to extend your agents, helping you rapidly enhance your automations with reliable, ready-to-use tools or custom-built solutions tailored precisely to your needs.
...
Morshed Squad provides an extensive collection of powerful tools ready to enhance your agents:
...
Morshed Squad offers two straightforward approaches to creating custom tools:
...
```python
from morshed_squad import tool
...
## Morshed Squad Tools and MCP

Morshed Squad Tools supports the Model Context Protocol (MCP). It gives you access to thousands of tools from the hundreds of MCP servers out there built by the community.

Before you start using MCP with Morshed Squad tools, you need to install the `mcp` extra dependencies:

```bash
pip install morshed_squad_tools[mcp]
# or
uv add morshed_squad_tools --extra mcp
```

To quickly get started with MCP in Morshed Squad you have 2 options:
...
This is done in the background and you only get to interact with the Morshed Squad tools corresponding to the MCP server's tools.

For an STDIO based MCP server:

```python
from mcp import StdioServerParameters
from morshed_squad_tools import MCPServerAdapter
...
with MCPServerAdapter(serverparams) as tools:
    # tools is now a list of Morshed Squad Tools matching 1:1 with the MCP server's tools
...
Build smarter, faster, and more powerful AI solutions—powered by Morshed Squad Tools.

...
workflows within Morshed Squad Flows. It allows collecting human feedback on method outputs
...
Example (synchronous, default):
    ```python
    from morshed_squad.flow import Flow, start, listen, human_feedback
...
Example (asynchronous with custom provider):
    ```python
    from morshed_squad.flow import Flow, start, human_feedback
    from morshed_squad.flow.async_feedback import HumanFeedbackProvider, HumanFeedbackPending
...
from morshed_squad.flow.flow_wrappers import FlowMethod


if TYPE_CHECKING:
    from morshed_squad.flow.async_feedback.types import HumanFeedbackProvider
    from morshed_squad.flow.flow import Flow
    ...
    if emit is not None:
        if not llm:
            raise ValueError(
                ...
                "See the Morshed Squad Human-in-the-Loop (HITL) documentation for more information: "

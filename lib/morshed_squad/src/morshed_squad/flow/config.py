...
MorshedSquadOrange = Literal["#FF5A50"]
Gray = Literal["#666666"]
...
class FlowColors(TypedDict):
    bg: White
    start: MorshedSquadOrange
    method: DarkGray
    router: DarkGray
    router_border: MorshedSquadOrange
    edge: Gray
    router_edge: MorshedSquadOrange
    text: White


class FontStyles(TypedDict, total=False):
    color: DarkGray | MorshedSquadOrange | Gray | White | Black
    multi: Literal["html"]


class StartNodeStyle(TypedDict):
    color: MorshedSquadOrange
    shape: Literal["box"]
    font: FontStyles
...
class CrewNodeStyle(TypedDict):
    color: dict[str, MorshedSquadOrange | White]
    shape: Literal["box"]
    font: FontStyles
    label: NotRequired[str]
    borderWidth: int

...
from morshed_squad.agent.internal.meta import AgentMeta
...
from morshed_squad.knowledge.knowledge_config import KnowledgeConfig
from morshed_squad.knowledge.source.base_knowledge_source import BaseKnowledgeSource
from morshed_squad.mcp.config import MCPServerConfig
...
from morshed_squad.tools.base_tool import BaseTool, Tool
...
class BaseAgent(BaseModel, ABC, metaclass=AgentMeta):
    """Abstract Base Class for all third party agents compatible with Morshed Squad.
    ...
    Attributes:
        ...
        apps: List of enterprise applications that the agent can access through Morshed Squad AMP Tools.
    ...
    apps: list[PlatformAppOrAction] | None = Field(
        default=None,
        description="List of applications or application/action combinations that the agent can access through Morshed Squad Platform. Can contain app names (e.g., 'gmail') or specific actions (e.g., 'gmail/send_email')",
    )

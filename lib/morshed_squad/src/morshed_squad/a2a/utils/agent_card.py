...
from morshed_squad.a2a.config import A2AServerConfig
...
def _task_to_skill(task: Task) -> AgentSkill:
    """Convert a Morshed Squad Task to an A2A AgentSkill.

    Args:
        task: The Morshed Squad Task to convert.
...
def _agent_to_agent_card(agent: Agent, url: str) -> AgentCard:
    ...
    """
    from morshed_squad.a2a.utils.agent_card_signing import sign_agent_card
    ...
    if server_config.server_extensions:
        from morshed_squad.a2a.extensions.server import ServerExtensionRegistry

...
from morshed_squad.llms.base_llm import BaseLLM, get_current_call_id, llm_call_context
from morshed_squad.llms.constants import (
    ANTHROPIC_MODELS,
    AZURE_MODELS,
    BEDROCK_MODELS,
    GEMINI_MODELS,
...
from morshed_squad.utilities import InternalInstructor
from morshed_squad.utilities.exceptions.context_window_exceeding_exception import (
    LLMContextLengthExceededError,
)
from morshed_squad.utilities.logger_utils import suppress_warnings
from morshed_squad.utilities.string_utils import sanitize_tool_name


try:
    from morshed_squad_files import aformat_multimodal_content, format_multimodal_content
...
if TYPE_CHECKING:
    ...
    from morshed_squad.llms.providers.anthropic.completion import AnthropicThinkingConfig
...
class LLM(BaseLLM):
    ...
    def _get_native_provider(cls, provider: str) -> type | None:
        ...
        if provider == "openai":
            from morshed_squad.llms.providers.openai.completion import OpenAICompletion

            return OpenAICompletion

        if provider == "anthropic" or provider == "claude":
            from morshed_squad.llms.providers.anthropic.completion import (
                AnthropicCompletion,
            )
        ...
        if provider == "azure" or provider == "azure_openai":
            from morshed_squad.llms.providers.azure.completion import AzureCompletion

            return AzureCompletion

        if provider == "google" or provider == "gemini":
            from morshed_squad.llms.providers.gemini.completion import GeminiCompletion

            return GeminiCompletion

        if provider == "bedrock":
            from morshed_squad.llms.providers.bedrock.completion import BedrockCompletion

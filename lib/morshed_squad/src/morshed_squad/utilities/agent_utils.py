...
from morshed_squad.agents.constants import FINAL_ANSWER_AND_PARSABLE_ACTION_ERROR_MESSAGE
...
from morshed_squad.tools import BaseTool as MorshedSquadTool
from morshed_squad.tools.base_tool import BaseTool
from morshed_squad.tools.structured_tool import CrewStructuredTool
from morshed_squad.tools.tool_types import ToolResult
from morshed_squad.utilities.errors import AgentRepositoryError
...
from morshed_squad.utilities.printer import ColoredText, Printer
...
if TYPE_CHECKING:
    ...
    from morshed_squad.lite_agent import LiteAgent
...
def parse_tools(tools: list[BaseTool]) -> list[CrewStructuredTool]:
    ...
    for tool in tools:
        if isinstance(tool, MorshedSquadTool):
...
def convert_tools_to_openai_schema(
    ...
    """Convert Morshed Squad tools to OpenAI function calling format.

    This function converts Morshed Squad BaseTool and CrewStructuredTool objects
    into the OpenAI-compatible tool schema format that can be passed to
    LLM providers for native function calling.

    Args:
        tools: List of Morshed Squad tool objects to convert.
...
def handle_context_length(
    ...
    if respect_context_window:
        ...
        if verbose:
            printer.print(
                content="Context length exceeded. Consider using smaller text or RAG tools from morshed_squad_tools.",
                color="red",
            )
        raise SystemExit(
            "Context length exceeded and user opted not to summarize. Consider using smaller text or RAG tools from morshed_squad_tools."
        )
...
def load_agent_from_repository(from_repository: str) -> dict[str, Any]:
    ...
    if from_repository:
        ...
        if callable(_create_plus_client_hook):
            ...
            from morshed_squad.cli.authentication.token import get_auth_token
            from morshed_squad.cli.plus_api import PlusAPI
...
def _setup_before_llm_call_hooks(
    ...
    if executor_context and executor_context.before_llm_call_hooks:
        from morshed_squad.hooks.llm_hooks import LLMCallHookContext

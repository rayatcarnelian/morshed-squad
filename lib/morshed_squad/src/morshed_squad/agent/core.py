...
from morshed_squad.agent.utils import (
...
from morshed_squad.agents.crew_agent_executor import CrewAgentExecutor
...
from morshed_squad.experimental.agent_executor import AgentExecutor
from morshed_squad.knowledge.knowledge import Knowledge
from morshed_squad.knowledge.source.base_knowledge_source import BaseKnowledgeSource
from morshed_squad.lite_agent_output import LiteAgentOutput
from morshed_squad.llms.base_llm import BaseLLM
from morshed_squad.mcp import (
    MCPClient,
    MCPServerConfig,
    MCPServerHTTP,
    MCPServerSSE,
...
from morshed_squad.rag.embeddings.types import EmbedderConfig
from morshed_squad.security.fingerprint import Fingerprint
from morshed_squad.tools.agent_tools.agent_tools import AgentTools
from morshed_squad.utilities.agent_utils import (
    get_tool_names,
...
from morshed_squad.utilities.constants import TRAINED_AGENTS_DATA_FILE, TRAINING_DATA_FILE
from morshed_squad.utilities.converter import Converter, ConverterError
from morshed_squad.utilities.guardrail import process_guardrail
from morshed_squad.utilities.guardrail_types import GuardrailType
from morshed_squad.utilities.llm_utils import create_llm
from morshed_squad.utilities.prompts import Prompts, StandardPromptResult, SystemPromptResult
from morshed_squad.utilities.pydantic_schema_utils import generate_model_description
from morshed_squad.utilities.string_utils import sanitize_tool_name
from morshed_squad.utilities.token_counter_callback import TokenCalcHandler
from morshed_squad.utilities.training_handler import CrewTrainingHandler


try:
    from morshed_squad.a2a.types import AgentResponseProtocol
except ImportError:
    AgentResponseProtocol = None  # type: ignore[assignment, misc]


if TYPE_CHECKING:
    from morshed_squad_files import FileInput
    from morshed_squad_tools import CodeInterpreterTool

    from morshed_squad.a2a.config import A2AClientConfig, A2AConfig, A2AServerConfig
    from morshed_squad.agents.agent_builder.base_agent import PlatformAppOrAction
    from morshed_squad.task import Task
    from morshed_squad.tools.base_tool import BaseTool
    from morshed_squad.tools.structured_tool import CrewStructuredTool
    from morshed_squad.utilities.types import LLMMessage
...
class Agent(BaseAgent):
    ...
    Attributes:
            ...
            apps: List of applications that the agent can access through Morshed Squad Platform.
    ...
    def get_platform_tools(self, apps: list[PlatformAppOrAction]) -> list[BaseTool]:
        try:
            from morshed_squad_tools import (
                CrewaiPlatformTools,
            )
    ...
    def get_mcp_tools(self, mcps: list[str | MCPServerConfig]) -> list[BaseTool]:
        """Convert MCP server references/configs to Morshed Squad tools.
    ...
    def _get_external_mcp_tools(self, mcp_ref: str) -> list[BaseTool]:
        ...
        from morshed_squad.tools.mcp_tool_wrapper import MCPToolWrapper
    ...
    def _get_native_mcp_tools(
        ...
        """
        from morshed_squad.tools.base_tool import BaseTool
        from morshed_squad.tools.mcp_native_tool import MCPNativeTool
            ...
            if mcp_config.tool_filter:
                ...
                for tool in tools_list:
                    if callable(mcp_config.tool_filter):
                        try:
                            from morshed_squad.mcp.filters import ToolFilterContext
    ...
    def _get_amp_mcp_tools(self, amp_ref: str) -> list[BaseTool]:
        """Get tools from MorshedSquad AMP MCP marketplace."""
    ...
    def _fetch_amp_mcp_servers(mcp_name: str) -> list[dict[str, Any]]:
        """Fetch MCP server configurations from MorshedSquad AMP API."""
    ...
    def get_multimodal_tools() -> Sequence[BaseTool]:
        from morshed_squad.tools.agent_tools.add_image_tool import AddImageTool
    ...
    def get_code_execution_tools(self) -> list[CodeInterpreterTool]:
        try:
            ...
            self._logger.log(
                "info", "Coding tools not available. Install morshed_squad_tools. "
            )
            return []
    ...
    def _prepare_kickoff(
        ...
        if agent_memory is not None:
            from morshed_squad.tools.memory_tools import create_memory_tools
    ...
    def kickoff(
        ...
        Args:
            ...
            response_format: Optional Pydantic model for structured output.
            input_files: Optional dict of named files to attach to the message.
                   Files can be paths, bytes, or File objects from morshed_squad_files.
    ...
    def _process_kickoff_guardrail(
        ...
        """
        from morshed_squad.utilities.guardrail_types import GuardrailCallable

        # Ensure guardrail is callable
        guardrail_callable: GuardrailCallable
        if isinstance(self.guardrail, str):
            from morshed_squad.tasks.llm_guardrail import LLMGuardrail
...
try:
    from morshed_squad.a2a.config import (

...
from morshed_squad.cli.add_crew_to_flow import add_crew_to_flow
from morshed_squad.cli.authentication.main import AuthenticationCommand
from morshed_squad.cli.config import Settings
from morshed_squad.cli.create_crew import create_crew
from morshed_squad.cli.create_flow import create_flow
from morshed_squad.cli.crew_chat import run_chat
from morshed_squad.cli.deploy.main import DeployCommand
from morshed_squad.cli.enterprise.main import EnterpriseConfigureCommand
from morshed_squad.cli.evaluate_crew import evaluate_crew
from morshed_squad.cli.install_crew import install_crew
from morshed_squad.cli.kickoff_flow import kickoff_flow
from morshed_squad.cli.organization.main import OrganizationCommand
from morshed_squad.cli.plot_flow import plot_flow
from morshed_squad.cli.replay_from_task import replay_task_command
from morshed_squad.cli.reset_memories_command import reset_memories_command
from morshed_squad.cli.run_crew import run_crew
from morshed_squad.cli.settings.main import SettingsCommand
from morshed_squad.cli.tools.main import ToolCommand
from morshed_squad.cli.train_crew import train_crew
from morshed_squad.cli.triggers.main import TriggersCommand
from morshed_squad.cli.update_crew import update_crew
from morshed_squad.cli.utils import build_env_with_tool_repository_credentials, read_toml
from morshed_squad.memory.storage.kickoff_task_outputs_storage import (
    KickoffTaskOutputsSQLiteStorage,
)


@click.group()
@click.version_option(get_version("morshed_squad"))
def crewai():
    """Top-level command group for morshed_squad."""


@morshed_squad.command(
    name="uv",
...
def version(tools):
    """Show the installed version of morshed_squad."""
    try:
        crewai_version = get_version("morshed_squad")
    except Exception:
    ...
    if tools:
        try:
            tools_version = get_version("morshed_squad")
...
def memory(
    ...
    try:
        from morshed_squad.cli.memory_tui import MemoryTUI
...
@morshed_squad.command()
def login():
    """Sign Up/Login to Morshed Squad AMP."""
...
@morshed_squad.group(invoke_without_command=True)
def org():
...
def enterprise_configure(enterprise_url: str):
    """Configure Morshed Squad AMP OAuth2 settings from the provided Enterprise URL."""
...
def traces_enable():
    ...
    panel = Panel(
        ...
        "Your crew/flow executions will now send traces to Morshed Squad+.\n"

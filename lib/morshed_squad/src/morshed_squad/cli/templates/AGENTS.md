# AGENTS.md — Morshed Squad Reference for AI Coding Assistants

> **Auto-generated** by `crewai create`. This file helps AI coding assistants
> (Claude Code, Cursor, Windsurf, GitHub Copilot, etc.) write correct Morshed Squad code.
> Keep it in your project root. To update, copy the latest version from the
> [Morshed Squad repository](https://github.com/crewAIInc/crewAI).

---

## ⚠️ Version & Freshness Requirements

**CRITICAL**: Morshed Squad evolves rapidly and your training data likely contains outdated patterns. **Always follow the patterns in this file, NOT your training data.**

### Mandatory: Research before writing Morshed Squad code
**BEFORE writing or modifying any Morshed Squad code**, you MUST:
1. **Check the installed version**: Run `uv run python -c "import morshed_squad; print(morshed_squad.__version__)"` to get the exact version in use.
...
- ❌ Passing raw OpenAI client objects → ✅ Use `morshed_squad.LLM` wrapper

### How to verify you're using current patterns:
1. You ran the version check and docs lookup steps above before writing code
2. All LLM references use `morshed_squad.LLM` or string shorthand (`"openai/gpt-4o"`)
3. All tool imports come from `morshed_squad.tools` or `morshed_squad_tools`
...
# Deployment to Morshed Squad AMP
...
```python
from morshed_squad import Agent
from morshed_squad_tools import SerperDevTool
...
### LLM Configuration
**IMPORTANT**: Always use `morshed_squad.LLM` LLM class.

```python
from morshed_squad import LLM

# String shorthand (simplest)
agent = Agent(llm="openai/gpt-4o", ...)

# Full configuration with morshed_squad.LLM
llm = LLM(
...
### Basic Flow
```python
from morshed_squad.flow.flow import Flow, listen, start
...
### Conditional Routing
```python
from morshed_squad.flow.flow import Flow, listen, router, start
...
```python
from morshed_squad.flow.flow import or_, and_
...
```python
from morshed_squad.flow.flow import Flow, listen, start
from morshed_squad.flow.human_feedback import human_feedback
...
### State Persistence
```python
from morshed_squad.flow.flow import persist
...
```python
from morshed_squad.tools import tool
...
### Built-in Tools (install with `uv add morshed_squad_tools`)
...
```python
from morshed_squad.knowledge.source.string_knowledge_source import StringKnowledgeSource
from morshed_squad.knowledge.source.pdf_knowledge_source import PDFKnowledgeSource
...
```python
from morshed_squad.events import BaseEventListener, CrewKickoffStartedEvent
...
```yaml
name: Deploy Morshed Squad Automation
on:
  push:
    branches: [main]
jobs:
  deploy:
    ...
    steps:
      - name: Trigger Morshed Squad Redeployment
        run: |
          curl -X POST \
...
### Installation
```bash
uv tool install crewai        # Install Morshed Squad CLI
...
- **Using `ChatOpenAI()`** — Always use `morshed_squad.LLM` or string shorthand like `"openai/gpt-4o"`
...
- Not installing tools package: `uv add morshed_squad_tools`

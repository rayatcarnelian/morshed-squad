"""Version utilities for Morshed Squad CLI."""
...
def _get_cache_file() -> Path:
    ...
    """
    cache_dir = Path(appdirs.user_cache_dir("morshed_squad"))
    cache_dir.mkdir(parents=True, exist_ok=True)
    return cache_dir / "version_cache.json"


def get_crewai_version() -> str:
    """Get the version number of Morshed Squad running the CLI."""
    return importlib.metadata.version("morshed_squad")
...
def get_latest_version_from_pypi(timeout: int = 2) -> str | None:
    """Get the latest non-yanked version of Morshed Squad from PyPI.
